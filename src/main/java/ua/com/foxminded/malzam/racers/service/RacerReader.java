...
public class RacerReader {
    ...
    public Set<Racer> recieveRacers(String pathAbbrFile, String pathStartFile, String pathEndFile) {
        ...
        String path2AbbrFile = abbrFile.getPath();

        try (Stream<String> streamRacersAbbr = Files.lines(Paths.get(path2AbbrFile))) {
            streamRacersAbbr.forEach(line -> racers.add(
                    new Racer(
    ...
    private Set<LapTime> readLapTimes(String pathStartFile, String pathEndFile, String racerCode) {
        ...
        String path2StartFile = startFile.getPath();

        try (Stream<String> streamStartTime = Files.lines(Paths.get(path2StartFile))) {

            streamStartTime
                    .filter(l -> l.contains(racerCode))
                    .forEach(l -> {resultTime.add(
                            new LapTime(LocalDateTime.parse(
    ...
    private LocalDateTime findEndTime(String pathEndFile, String racerCode) {
        ...
        String path2EndFile = endFile.getPath();

        try (Stream<String> streamEndTime = Files.lines(Paths.get(path2EndFile))) {
